...
const FLOWER_COUNT = 35; // Increased slightly for better mix

const TetEffect: React.FC = () => {
  // Generate random flowers once with type 'mai' or 'dao'
  const flowers = useMemo(() => {
    return Array.from({ length: FLOWER_COUNT }).map((_, i) => ({
      id: i,
      left: Math.random() * 100, // 0-100% width
      animationDuration: 5 + Math.random() * 10, // 5-15s fall time
      animationDelay: Math.random() * 5, // 0-5s delay
      size: 15 + Math.random() * 20, // 15-35px size
      swayDuration: 2 + Math.random() * 2, // Sway speed
      type: Math.random() > 0.5 ? 'mai' : 'dao', // Randomly choose type
    }));
  }, []);
